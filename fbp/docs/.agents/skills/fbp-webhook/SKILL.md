---
name: fbp-webhook
description: Implement and operate webhook-driven integrations in FBP, including rule registration, LINE reply/push URL formatting, and end-to-end verification.
---
...
- Flex Messageを返信する action_class は、`["reply_flex" => ["alt_text" => "...", "contents" => <bubble|carousel>], "handled" => true]` を返す。公開URLはFlex Message内のURI actionへ設定し、カード全体やボタンから直接開ける形を優先する。
...
- Webhook 起点で誘導する公開側画面では、ブラウザ標準の `confirm()` / `window.confirm()` を使わない。削除・送信前確認・重要操作の確認は、FBP標準の `show_multi_dialog()` + `ajax-link` + `data-form` による確認ダイアログで実装する。
...
- LINE webhook 起点の公開側でも、削除確認などを `confirm()` / `window.confirm()` に逃がさない。確認UIは FBP標準ダイアログに統一する。
