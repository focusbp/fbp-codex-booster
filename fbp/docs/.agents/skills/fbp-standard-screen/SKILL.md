...
- システム画面の「WEB」パネルにある「標準画面レスポンシブ」で、アプリ全体の表示を切り替える。`standard_screen_responsive` は `0:レスポンシブ` / `1:パソコン`。未設定・空・不正値は0として扱い、保存後の画面再読込で反映する。
- 「検索＋一覧」「マニュアルソート」のメイン一覧は、画面幅700px以下で1件ごとのカードに切り替わる。既存の `row_title` / `row_value` を使い、検索条件は1列、日付・日時の範囲は上下に並べる。
- 上記のカード化は0の場合だけ有効。1では狭い画面でも従来の表形式・検索欄と最小幅800pxを維持し、ノート側の横スクロール設定を使う。共通ヘッダーも1では従来の表示になるため、管理画面内の独自画面にもヘッダーの選択は適用される。
- 既存の管理側・公開側Viewport設定は変更しない。700pxの判定はCSS表示領域幅に従う。公開ページ・ログイン画面、独自画面の本文はこの設定の対象外。
...
- 並び替え中の列幅固定は表形式のドラッグ用複製要素だけに行い、元の行に幅を残さない。画面幅だけで判断せず、パソコンモードを含めた実際の行表示形式を参照する。
