...
$assetDir = $skillDir . "/assets/web-commerce-basic";
$manifestFile = $assetDir . "/web-commerce-basic.json";
...
function run_cli(string $command, ?array $payload = null): array {
	...
	$tmp = null;
	if ($payload !== null) {
		$tmp = tempnam(sys_get_temp_dir(), "fbp_web_commerce_basic_");
		if ($tmp === false) {
...
foreach ($manifest["db_tables"] as $table) {
	...
	$dbIds[$tbName] = $dbId;
	$tableDefs[$tbName] = $table;
}

foreach ([
	"shop_product_variant" => "shop_product",
	"shop_customer_order_item" => "shop_customer_order",
] as $childTable => $parentTable) {
	if (isset($tableDefs[$childTable], $dbIds[$parentTable])) {
		$table = $tableDefs[$childTable];
		$table["parent_tb_id"] = $dbIds[$parentTable];
		run_cli("db_tables_edit", $table);
	}
}
...
$memberRows = run_cli("data_list", ["table" => "shop_member", "max" => 1])["items"] ?? [];
if ($resetData || count($memberRows) === 0) {
	foreach ($manifest["seed_members"] as $seed) {
		...
		$row["created_at"] = time();
		$row["updated_at"] = time();
		run_cli("data_add", ["table" => "shop_member", "data" => $row]);
	}
}

$categoryRows = run_cli("data_list", ["table" => "shop_product_category", "max" => 1])["items"] ?? [];
if ($resetData || count($categoryRows) === 0) {
	$categoryIds = [];
	foreach ($manifest["seed_categories"] as $seed) {
		$key = (string) ($seed["_key"] ?? "");
		$row = $seed;
		unset($row["_key"]);
		$id = (int) (run_cli("data_add", ["table" => "shop_product_category", "data" => $row])["id"] ?? 0);
		if ($key !== "" && $id > 0) {
			$categoryIds[$key] = $id;
		}
	}
	foreach ($manifest["seed_products"] as $seed) {
		$categoryKey = (string) ($seed["_category_key"] ?? "");
		if (!isset($categoryIds[$categoryKey])) {
			continue;
		}
		$product = $seed;
		$variants = $product["variants"] ?? [];
		unset($product["_key"], $product["_category_key"], $product["variants"]);
		$product["category_id"] = $categoryIds[$categoryKey];
		$product["created_at"] = time();
		$product["updated_at"] = time();
		$productId = (int) (run_cli("data_add", ["table" => "shop_product", "data" => $product])["id"] ?? 0);
		foreach ($variants as $variant) {
			$variant["parent_id"] = $productId;
			run_cli("data_add", ["table" => "shop_product_variant", "data" => $variant]);
		}
	}
}
...
echo json_encode([
	...
	"public_url_path" => "/public_pages*shop",
	"sample_login" => [
