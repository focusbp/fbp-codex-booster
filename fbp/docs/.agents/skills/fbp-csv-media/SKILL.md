...
- PDF変換などでアプリ独自の作業ファイルが必要な場合は `fbp-temp-files` に従い `$ctl->get_temp_dir()` を使う。固定のサーバーパスは使わない。`save_pdf()` / `res_saved_file()` 等の管理ファイルは各API所定の保存先を使う。一時用途のファイルは応答後・例外時・exit時に削除する。
